...
- **ai_insights.py** — AI-powered analytics engine: Isolation Forest anomaly detection (10-dim feature space), composite geopolitical risk scoring (0-100), correlation regime analysis, scenario intelligence, dynamic strategy recommendations, automated narrative generation
- **run_all.py** — orchestrates all 6 scripts sequentially; checks output files exist
- **terminal_dashboard.py** — live rich TUI dashboard: runs full pipeline (all 6 scripts), displays prices, regime state, VaR/CVaR, scenario outlook, regime correlations, AI insights panel with risk score, anomaly alerts, and recommendations in real-time; `--refresh` flag for continuous refresh every 5 min; shows PDF report status in footer
